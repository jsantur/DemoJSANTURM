...
document.addEventListener("DOMContentLoaded", function () {
	document.querySelectorAll(".hero-form form").forEach(form => {
		form.addEventListener("submit", async function (event) {
			...
			grecaptcha.ready(() => {
				grecaptcha.execute(rnz_forms.recaptcha_key, { action: "submit" }).then(async token => {
					
					if( document.getElementById('terminos') !== null ){
						let checkbox = document.getElementById('terminos'), label = checkbox.nextElementSibling;
					...
					if( document.getElementById('informacion') !== null ){
						let checkbox = document.getElementById('informacion'), label = checkbox.nextElementSibling;
